...
###########################################################################
#
#   This script outputs overlays of the mu and NPV term residuals for the
#     default used rho (FINE) and the other area rho is calculated over
#
###########################################################################
...
DownloadConfig = "/cluster/warehouse/ddobre/thesis/DeriveHLLHCJES_test2/DeriveHLLHCJES/macros/Download.config"

downloadC = ROOT.TEnv(DownloadConfig)
version = downloadC.GetValue("Version", "VPF1")

term = sys.argv[1]
histogram_to_compare = sys.argv[2]
...
print histogram_to_compare
print "\n-----> Files to compare:"
for temp in inputfiles:
  print temp
print ""
...
###########################################################################
# Gather hists from root files
for target in inputfiles:

  myfile = ROOT.TFile(target)
  tempHist = myfile.Get(histogram_to_compare)
...
hists[0].Draw()
hists[0].SetTitle("Comparing residuals for the {0} term".format(term.split("Term")[0]))
hists[0].SetLineColor(kBlack)
hists[0].SetMarkerColor(kBlack)
hists[0].GetFunction("{0}_myfit".format(histogram_to_compare)).SetLineColor(kBlack)
#hists[0].GetYaxis().SetRangeUser(0.3,1.2)
algo = (inputfiles[0].split(term)[1]).split(version)[0]
leg.AddEntry(hists[0], algo,"l")

outputname = algo

for k in xrange(1,len(hists)):
  hists[k].Draw("same")
  hists[k].SetLineColor(Colours[k])
  hists[k].SetMarkerColor(Colours[k])
  hists[k].GetFunction("{0}_myfit".format(histogram_to_compare)).SetLineColor(Colours[k])

  algo = (inputfiles[k].split(term)[1]).split(version)[0]
  outputname = outputname + "_" + algo
  leg.AddEntry(hists[k], algo ,"l")
  canvas.Update()

leg.Draw()
canvas.SaveAs("resolution_plots/residual_comparison/{0}_{1}.eps".format(term, outputname))
canvas.Delete()

# residual_plots/residual_comparison
