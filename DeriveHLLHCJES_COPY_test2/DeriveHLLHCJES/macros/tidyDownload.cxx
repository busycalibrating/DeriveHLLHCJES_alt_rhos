...
void tidyDownload(const char* rtcc = "", const char* ver=""){
  ...
  downloadE->Print();
  TString User = downloadE->GetValue("User","ddobre");
  ...
  for (int i = 0 ; i < Rtags.size(); ++i){
    ...
    std::cout<<"ls "+InputFolder+"/temp/*/* -r > "+InputFolder+"/temp/tempfilelist.txt"<<std::endl;
    std::cout<<"cat "+InputFolder+"/temp/tempfilelist.txt | xargs hadd "+InputFolder+"/"+Rtags.at(i)+".root &> "+InputFolder+"/temp/shittyhaddoutput.txt"<<std::endl;
    ...
    system((const char*)("ls "+InputFolder+"/temp/*/* -r > "+InputFolder+"/temp/tempfilelist.txt"));
    system((const char*)("cat "+InputFolder+"/temp/tempfilelist.txt | xargs hadd "+InputFolder+"/"+Rtags.at(i)+".root &> "+InputFolder+"/temp/shittyhaddoutput.txt"));
