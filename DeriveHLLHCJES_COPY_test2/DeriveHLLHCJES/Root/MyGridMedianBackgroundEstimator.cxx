//STARTHEADER
// $Id: MyGridMedianBackgroundEstimator.cxx 323529 2016-06-20 20:28:30Z alkire $
//
...
void MyGridMedianBackgroundEstimator::set_particles(const vector<PseudoJet> & particles) {
...
double MyGridMedianBackgroundEstimator::rho() const {
...
// given area.
double MyGridMedianBackgroundEstimator::sigma() const{
  verify_particles_set();
...
// determination.
double MyGridMedianBackgroundEstimator::rho(const PseudoJet & jet)  {
  verify_particles_set();
...
double MyGridMedianBackgroundEstimator::sigma(const PseudoJet & jet){
  verify_particles_set();
  double rescaling = (_rescaling_class == 0) ? 1.0 : (*_rescaling_class)(jet);
...
void MyGridMedianBackgroundEstimator::verify_particles_set() const {
  if (!_has_particles) throw Error("MyGridMedianBackgroundEstimator::rho() or sigma() called without particles having been set");
}
...
// description
//----------------------------------------------------------------------
string MyGridMedianBackgroundEstimator::description() const { 
  ostringstream desc;
  desc << "MyGridMedianBackgroundEstimator, with grid extension |y| < " << _ymax 
...
// actual computation
void MyGridMedianBackgroundEstimator::set_rescaling_class(const FunctionOfPseudoJet<double> * rescaling_class_in) {
...
//----------------------------------------------------------------------
// configure the grid
void MyGridMedianBackgroundEstimator::setup_grid() {
  ...
  assert(_ymax>0 && 2 * (_ymax - _ymin) >= _requested_grid_spacing);

  // this grid-definition code is becoming repetitive -- it should
  // probably be moved somewhere central...
  double ny_double = 2 * (_ymax-_ymin) / _requested_grid_spacing;
  _ny = int(ny_double+0.5);
  _dy = 2 * (_ymax-_ymin) / _ny;
...
int MyGridMedianBackgroundEstimator::igrid(const PseudoJet & p) const {
  ...
  float prap = p.rap();
  if(prap<-_ymax||prap>_ymax||fabs(prap)<_ymin) return -1;
  int iy = int(floor( (p.rap() + _ymax - (p.rap() > 0.) * 2. * _ymin) / _dy ));
