...
void algorithm_chain(EL::Job & job, std::string JESconfig){
  ...
  // -----REMOVE-----v (Don't need unmatch to do derivations. Use to look at jets not matches to a truth jet. Exists inside the tree-maker)
...
int main( int argc, char* argv[] ) {
  ...
  if (TString(grid).Contains("true")) {
    ...
    std::cout << "Looking for files in inputFileList... " << inputFileList << std::endl;
    ifstream f;	string s;
  ...
  SH::scanDQ2 (sh, s);
    ...
    job.options()->setDouble (EL::Job::optMaxEvents, 10); // Set how many events you want to take
    ...
    driver.options()->setDouble("nc_nFiles", 3);
    ...
    // Only runs this if running locally!
